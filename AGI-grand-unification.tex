...
\begin{itemize}
	...
	\item \textbf{Hopfield 网络}的权重 定义了一个 能量地势 (energy landscape)，它可以看成是一个 implicit 的 \textbf{概率分布}。 透过 Hopfield 网络的 learning，可以改变这个概率分布。 但这需要修改 Hopfield 网络的算法，将 能量 诠释成 概率。

	\item 根据 ``Hopfield Network is All You Need'' 论文，现代 Hopfield 网络的 update rule 跟 \textbf{Transformer} 重合。 换句话说，每执行一次 Transformer，就会趋向 Hopfield 的能量最低点。 
