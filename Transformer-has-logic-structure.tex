...
Propositions are made up of \textbf{atomic concepts}, but here, at the sub-propositional level, atoms cannot be permuted freely:
\begin{equation}
...
\end{equation}
The crucial point here is that propositions are made up of atoms ($\atom$), which is achieved in the Transformer by \textbf{adding} vectors (that represent atomic concepts) together.  Note also that the Transformer is equivariant, so we must add ``positional encoding'' to each word.
